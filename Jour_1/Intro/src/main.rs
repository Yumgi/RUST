...
fn main(){
    ...
    //1.  les variables RUST
     ...
     println!("Hello la température à lyon {}", temperature);
      ...
      println!("le chien  de lyon  a {} ans", age_chien);  
       println!("Hello {} , la temperature à lyon est de {}", nom, temperature);
    ...
    let nombre = 16;
     if nombre %2 == 0 {
       ...
       for(i,nom) in noms.iter().enumerate(){
        ...
        // Usage enumerate() dans un cas réel
   ...
   /*
              POO : programmation orienté objet -> classe => represente un objet => des propriétes et actions
                 ...
                 => class Voiture {
                        ...
                        mon_objet2.couleur = "rose"
                        mon_objet.couleur="vert"
                        mon_objet.puissance="120CV"
                        mon_objet.freiner()
                        mon_objet().demarrer()
  ...
  // usage de struct : on crée le model/la structure 
   ...
   // usage de struct => on crée une instance de la structure 
